...
async function start() {
    ...
    const handle: PanelHandle = {
      ...
      load(key: string, blendTime = 0) {
        ...
        this.visualizer.loadPreset(
          clonePresetGraphForButterchurn(preset),
          blendTime,
        );
  ...
  const optionsHud = createOptionsSummaryHud("Movement · debug", () => {
    ...
    const rows = buildRows([
      ...
      { label: "Opacity (grid)", value: opGrid },
    ]);
    const hint =
      ...
      "A audio · I labels · O HUD · Q quality · Y intensity · . , opacity · N/B compare scene (PgUp/PgDn too) · window.setDebugScene(i) · window.setPanel(i,key)" +
      "</div>";
    return rows + hint;
  });
