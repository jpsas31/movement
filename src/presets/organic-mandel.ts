// organic-mandel — autonomous 3D Mandelbox fly-through with cyclic palette mapping.
//
// Visual: a virtual camera navigates through a Mandelbox fractal in real-time,
// steering autonomously based on proximity to structure boundaries. The comp shader
// maps luminance through a 5-stop cyclic palette (purple → navy → green → gold → red)
// and blends structure edges with a shifted version of the same palette for depth.
//
// Lineage: compiled from flexi + amandio c - organic12-3d-2.milk (warp/frame/init/pixel)
// + custom cyclic-palette comp shader.
// Note: the aggression parameter is accepted for registry compatibility but currently
// has no effect — all tiers produce identical output.
...
const PAL3 = "vec3(0.024, 0.451, 0.008)"; // #067302 green
const PAL4 = "vec3(0.651, 0.490, 0.012)"; // #A67D03 gold
const PAL5 = "vec3(0.651, 0.012, 0.012)"; // #A60303 red

// Warp shader (compiled from flexi + amandio c - organic12-3d-2.milk):
// Casts a ray into the Mandelbox fractal using the rotation matrix in q20–q28.
// Each pixel's UV displacement is determined by the fractal boundary distance —
// pixels near structure edges warp strongly; open space warps gently. This creates
// the organic tunnelling effect where the camera appears to fly through structure.
const ORGANIC_WARP =
...
// Init equations (compiled from organic12-3d-2.milk):
// Resets all state registers and finds a valid camera start position by sampling
// random directions and ray-marching until a position with dist > 0.06 is found.
// Also zeros megabuf/gmegabuf (10k entries each) to clear previous run state.
const ORGANIC_INIT =
...
// Pixel equations: applies camera rotation (q11=v1) and lateral translation
// (q12/q13 → dx/dy) scaled by zoom factor (q16). warp=0 disables the built-in
// butterchurn warp so the custom ORGANIC_WARP shader handles all UV displacement.
const ORGANIC_PIXEL =
  "a.warp=0;a.zoom=1;a.dx=div(-a.q12,a.q16)*(1+0*pow(a.x-.5,2));a.dy=div(a.q13,a.q16)*(1+0*pow(a.y-.5,2));a.rot=a.q11;";

export function createOrganicMandel(_aggression: VizIntensity): PresetWithBase {
  return {
    version: 2,
    baseVals: {
      rating: 5,
    ...
    init_eqs_str: ORGANIC_INIT,
    frame_eqs_str: ORGANIC_FRAME,
    pixel_eqs_str: ORGANIC_PIXEL,
    warp: ORGANIC_WARP,
    comp: ORGANIC_COMP,
  };
}
